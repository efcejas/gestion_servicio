"""
Script para cargar datos de ejemplo en la base de datos local
...
def crear_preinformes():
    ...
    for data in preinformes:
        ...
        if Preinforme.objects.filter(numero_estudio=data['numero_estudio']).exists():
            print(f"- Ya existe preinforme {data['numero_estudio']}")
            continue
        ...
        preinforme = Preinforme.objects.create(
            ...
            estado=data['estado'],
            contenido_html=data['plantilla'].contenido,
        )
        ...
        if data['estado'] == 'finalizado':
            revision = RevisionPreinforme.objects.create(
                ...
                puntuacion=8,
                comentarios='Informe correcto, bien estructurado.',
                estado_revision='aprobado',
            )
            preinforme.revisor = revisor
            preinforme.save()
            print(f"✓ Creado preinforme {data['numero_estudio']} ({data['sistema_destino']}) - FINALIZADO")
        else:
            print(f"✓ Creado preinforme {data['numero_estudio']} ({data['sistema_destino']}) - {data['estado']}")
...
def main():
    ...
    print("="*60)
    print("\nResumen:")
    print(f"- {TipoEstudio.objects.count()} tipos de estudio")
    print(f"- {Region.objects.count()} regiones")
    print(f"- {PlantillaPreinforme.objects.count()} plantillas")
    print(f"- {Preinforme.objects.count()} preinformes")
    print("\nPuedes probar copiando los informes finalizados (2026-0001 a 2026-0004)")
    print("Verás la diferencia entre NetTerm (texto plano) y EGES (con formato)")
