...
class ServicioDistribucionTest(TestCase):
    ...
    def test_transicion_agosto_octubre_viernes_combina_r1_y_r2(self):
        """Con restricciones, la doble cobertura V/D combina R1 + R2."""
        from .models import ConfiguracionTipoGuardia as CTG

        anio = timezone.localdate().year + 1
        tipo_a = ConfiguracionTipoGuardia.objects.create(
            nombre='Viernes transición A',
            hora_inicio=datetime.time(20, 0),
            hora_fin=datetime.time(8, 0),
            dias_semana='V',
            creado_por=self.jefe,
        ...
        por_fecha = {}
        for asignacion in AsignacionGuardia.objects.filter(
            fecha__year=anio,
            fecha__month=9,
            estado='BORRADOR',
        ).select_related('residente'):
            por_fecha.setdefault(asignacion.fecha, set()).add(
                asignacion.residente.anio_residencia
            )

        self.assertTrue(por_fecha)
        self.assertTrue(
            any('Período transitorio' in aviso for aviso in resultado['advertencias'])
        )
        for anios in por_fecha.values():
            self.assertEqual(len(anios), 2)
            self.assertIn('R1', anios)
            self.assertIn('R2', anios)

    def test_transicion_agosto_octubre_sabado_corresponde_a_r3(self):
        from .services import _anio_puede_cubrir_slot

        sabado_transicion = datetime.date(2027, 9, 4)

        self.assertTrue(_anio_puede_cubrir_slot('R3', sabado_transicion, False))
        self.assertFalse(_anio_puede_cubrir_slot('R2', sabado_transicion, False))
        self.assertFalse(_anio_puede_cubrir_slot('R1', sabado_transicion, False))

    def test_desde_noviembre_sabado_vuelve_a_corresponder_a_r2(self):
        from .services import _anio_puede_cubrir_slot

        sabado_noviembre = datetime.date(2027, 11, 6)

        self.assertTrue(_anio_puede_cubrir_slot('R2', sabado_noviembre, False))
        self.assertFalse(_anio_puede_cubrir_slot('R3', sabado_noviembre, False))
